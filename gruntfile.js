...
module.exports = function (grunt) {
		...
		nodemon: {
			dev: {
				...
				options: {
					...
					env: {
						//MONGO_SEED:true?// generate user and password
						NODE_ENV: 'development'
						...
						nodemon.on('config:update', function () {
							...
							setTimeout(function () {
								if (config.secureServer) {
									require('open')('https://localhost:' + config.port);
								} else {
	...
	grunt.task.registerTask('mkdir:logs', 'Task that makes sure logs directory exists.', function () {
		// Get the callback
		var done = this.async();

		grunt.file.mkdir(path.normalize(__dirname + '/logs'));
	...
	grunt.task.registerTask('mkdir:upload', 'Task that makes sure upload directory exists.', function () {
		...
		grunt.file.mkdir(path.normalize(__dirname + '/upload'));
	...
	grunt.task.registerTask('dropdb', 'drop the database', function () {
		...
		var mongoose = require('./config/lib/mongoose.js');
		mongoose.loadModels();
		var dataHelper = require('./mockData');

		mongoose.disconnect(function(){
			mongoose.connect(function(db){
				dataHelper.cleanAllData(
					function(){
	...
	grunt.registerTask('test', ['env:test', 'mkdir:logs', 'mkdir:upload', 'copy:localConfig', 'server', 'mochaTest', 'karma:unit', 'protractor']);
	grunt.registerTask('test:server', ['env:test', 'dropdb', 'server', 'mochaTest']);
	...
	grunt.registerTask('debug', ['env:dev', 'mkdir:logs', 'mkdir:upload','copy:localConfig', 'concurrent:debug']);
};
